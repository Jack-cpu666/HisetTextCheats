...
# ** MODIFIED JAVASCRIPT IN INTERFACE_HTML **
INTERFACE_HTML = """
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function createPeerConnection() {
        closeConnection();
        const pcConfig = { iceServers: iceServers }; // Use servers from the server
    ...
    socket.on('connect', () => { 
        ...
        socket.emit('request_webrtc_config'); // Ask server for ICE config
    });
    
    socket.on('webrtc_config', (config) => {
        iceServers = config.iceServers;
        console.log("Received WebRTC config:", iceServers);
        socket.emit('controller_ready'); // Now we are ready
    });
...
def handle_register_client(data):
    ...
    if data.get('token') == ACCESS_PASSWORD:
        ...
        # Don't emit client_ready yet, the controller will trigger the process.
    else:
        logger.warning(f"Failed registration attempt from SID {request.sid}")

# ** NEW HANDLER TO PROVIDE CONFIG **
...
def handle_webrtc_ice_candidate(data):
    # This now handles candidates from both client and controller
