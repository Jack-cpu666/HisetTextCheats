...
<head>
    ...
    <style>
        ...
        /* New styles for Text Input Mode */
...
<body class="bg-gray-200" tabindex="0">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // ... (WebRTC and control logic is the same) ...
            async function createPeerConnection() { /* ... same as before ... */ 
                ...
                newPc.onicecandidate = e => { if (e.candidate) socket.emit('webrtc_ice_candidate', { candidate: e.candidate.toJSON() }); };
                newPc.ontrack = e => { if (screenVideo.srcObject !== e.streams[0]) screenVideo.srcObject = e.streams[0]; };
                newPc.onconnectionstatechange = () => { if(newPc.connectionState === 'connected') updateStatus('status-connected', 'Remote PC Connected'); };
                return newPc;
            }
            ...
            socket.on('client_connected', (data) => { updateStatus('status-connecting', 'PC found, starting video...'); remoteDimensions = data.dimensions; socket.emit('controller_ready'); });
            ...
            socket.on('webrtc_offer', async (data) => { pc = await createPeerConnection(); await pc.setRemoteDescription(new RTCSessionDescription(data.offer)); const answer = await pc.createAnswer(); await pc.setLocalDescription(answer); socket.emit('webrtc_answer', { answer: pc.localDescription.toJSON() }); });
            ...
            toggleTextModeBtn.addEventListener('click', () => {
                ...
                if (body.classList.contains('text-mode')) {
                    injectionTextarea.focus();
                } else {
                    document.body.focus();
                }
            });
...
# ... (all existing handlers are the same) ...
@socketio.on('connect')
def handle_connect():
